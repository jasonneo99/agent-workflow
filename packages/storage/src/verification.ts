...
const fingerprintExpressions: Record<string, string> = {
  ...
  workflow_runs: "id::text || ':' || project_id::text || ':' || workflow_id || ':' || status || ':' || policy_snapshot_hash || ':' || executor_snapshot::text",
  workflow_tasks: "id::text || ':' || run_id::text || ':' || stage_id || ':' || agent_id || ':' || status || ':' || attempts::text || ':' || executor_snapshot::text",
...
function redactSnapshot(snapshot: StorageSnapshot): StorageSnapshot {
  return {
    ...snapshot,
    endpoint: {
      databaseUrl: redactStorageUrl(snapshot.endpoint.databaseUrl),
      redisUrl: redactStorageUrl(snapshot.endpoint.redisUrl),
      objectStorageEndpoint: redactStorageUrl(snapshot.endpoint.objectStorageEndpoint),
...
function sameEndpointWarnings(source: StorageEndpointSummary, target: StorageEndpointSummary): string[] {
  return storageEndpointWarnings(source, target, "source and target database URLs point to the same endpoint; this verifies one shared storage plane, not a migration copy");
}
