...
`release:check` is read-only. It verifies repository cleanliness and sync, bundle manifest/signature consistency, Trusted Publisher workflow configuration, package contents, npm published-version readiness, local npm auth status, and the standard validation/test commands. Use `--allow-current-version` when checking a post-release checkout where the local package version is already published. Use `--allow-dirty` only when testing the checker while editing. Use `--gate-run <candidate-run-id>` with optional `--baseline-run <baseline-run-id>` to block release promotion when a project-local evaluation gate fails.
