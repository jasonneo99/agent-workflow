...
stage-to-agent and agent-to-run relationships, stage nodes jump to the Stage
Matrix, and run nodes open the run detail page, so the visual becomes a
lightweight inspection surface. Use `runLimit=0` for definition-only diagrams or
